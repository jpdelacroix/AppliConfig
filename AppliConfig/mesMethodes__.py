...
def loadModel(structGlob):
    ...
    if debug:
        print("**** DEBUG ****** structGlob ==>",structGlob)
    ...
    if len(root)== 0:           
        for k,v in structGlob.items(): 
            if debug:
                print('<LOAD MODEL> clef:',k,'new:',v[0],'target:',v[1], 'clefTarget:',v[4], "type:",v[3])
            ...
            try:
                print("** TRY **","target:",target,"newNode:",newNode)
                newNode.insert_at(target,position='last-child',save=True)
            except:
                print("** EXCEPT **","target:",target,"newNode:",newNode)
        return (0) 
        ...
        for line in fic:
            ...
            if init:                #Premier passage (ligne)
                ...
                if line[-1] == ':' and line[x] != '-': 
                    if debug:
                        print('structGlob avec nb = ',nb,'level:',level,'===>',structGlob.items())
    ...
    if fichier_.strip() != '':
        ...
        for xx in root:
            ...
            if xx.is_leaf_node():
                ...
                #structGlobal[anc] = xx.get_level(),anc.split('|')[:-1],anc.split('|')[0:3]
                structGlobal[anc] = xx.get_level(),anc.split('|')[:-1]
        ...
        for clef in structGlobal:
            ...
            if structGlobal[clef][1] not in listeAffiliation:
                ...
                count = clef.count('|')
                listeEquation.append(clef.split('|')[count])
                if debug:
                ...
                try:
                    if debug:
                        print("Début du Try")
                    count = clef.count('|')
                    listeEquation[idx_found] = listeEquation[idx_found] + "|" + clef.split('|')[count]
                    print('structGlobal[clef]:',structGlobal[clef])
                    print('-- listeAffiliation -- :',listeAffiliation,'\n')
                    print('listeEquation',listeEquation)
                    if debug:
                    ...
                    #listeEquation[idx_found].append("|" + clef.split('|')[count])
                    print("*** General Exception Error ***")
                    #print('listeAffiliation[0] :',listeAffiliation[idx_found])
                    #print("clef.split('|')[count]",clef.split('|')[count])
